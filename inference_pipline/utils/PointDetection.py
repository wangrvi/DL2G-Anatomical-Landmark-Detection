...
def Get_featurePoints_one_image(args, Img_arr):
    ...
    image = tio.ScalarImage(tensor=torch.tensor(Img_arr).unsqueeze(0))
    resample_image = resample(image)
    screen_array = tensor2numpy_squeeze0(resample_image.data)

    end = time.time()
    # get feature points use space combinations: cross, corner_u, corner_t
    feature_points = PreScreen(screen_array, preScreenSize)
    feature_points = (np.array(feature_points) * resample_scale)

    if Aug:
        print("augment feature points, each point augment to ", augnum)
        feature_points_aug = augpoint(feature_points)
        # delete the repeat points and constraint the points in the range of the image
        feature_points = constraint_Points_coordinate(feature_points_aug)
    
    print(f"get feature point: {len(feature_points)}, use time: {time.time() - end}")
    
    return feature_points


def InferPoints_localSelect(templateafids, validpoints, threshold=30):
    '''
